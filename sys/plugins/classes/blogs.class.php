...
class blogs
{
    ...
    public static function editBlog($blogId, $title, $content, $preview){
        $sql = "UPDATE `blogs` SET `title` = ?, `content` = ?, `preview` = ? WHERE `id` = ?";
        $res = DB::me()->prepare($sql);
        return $res->execute(Array($title, $content, $preview, $blogId));
    }
}